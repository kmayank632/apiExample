...
fun decodeFileFromUri(
    ...
    val options = BitmapFactory.Options()
    options.inJustDecodeBounds = true
    MediaUtils().getBitmapFromUri(context, uri, options)
    options.inJustDecodeBounds = false
    ...
    return MediaUtils().getBitmapFromUri(context, uri, options)
}
 ...
 * @return Decoded bitmap
 */
fun decodeFileFromPath(path: String, dstWidth: Int, dstHeight: Int): Bitmap? {
    val options = BitmapFactory.Options()
    options.inJustDecodeBounds = true
...
/**
 * Get Image Quality Percent according to File Size
 * */
...
/**
 * Get aspect ratio size of image with conversion
 * */
fun getImageHgtWdt(context: Context, uri: Uri, maxHeight: Float, maxWidth: Float): Pair<Int, Int> {
    ...
    MediaUtils().getBitmapFromUri(context, uri, opt)
